...
local menuItems = {"PLAY  GAME", "OPTIONS", "EXTRAS"}
local menuItems2 = {"RESET SAVE", "BACK"}
local curArray = {}
...
function onCreate()
	...
	makeLuaSprite("selectorMenu", "menus/titleDX/pcport/selector", wideScreen, 12 * 6)
	...
	makeAnimatedLuaSprite("mario", "menus/titleDX/pcport/mario", 0, 0)
	...
	makeLuaSprite("bricks", "menus/titleDX/pcport/bricks", -wideScreen2, 0)
	...
	makeLuaSprite("version", "menus/titleDX/pcport/info", wideScreen, 0)
...
function onUpdate(elapsed)
	...
	if getTextString("curMenuGlobal") == "1" or getTextString("curMenuGlobal") == "-1" then
		...
		if keyJustPressed("accept") and canControl then
			...
			if daSelected == "PLAY  GAME" then
				playSound("pcport/coin")
				flicker(menuItem, 1, 0.05, true)
				runTimer("playGame", 1)
			elseif daSelected == "FREEPLAY" then
				playSound("pcport/stomp")
				flicker(menuItem, 1, 0.05, true)
