...
local variableTable
local queueSetTable
local queueSetPTable
local queueEaseTable
local queueEasePTable
...
function onCreatePost()
    ...
    addCharacterToList("dark-sonic-trans")
    addCharacterToList("dark-sonic-mad")
    addCharacterToList("dark-sonic", 1)
    precacheSound("tfbscream")
end
...
function addBlankMod(name,setValue)
addToList(variableTable,{name,setValue})
end

function getValue(name,value)
for i=1,#variableTable do
if variableTable[i][1] == name then
local hereYouGo = variableTable[i][2]
return hereYouGo
end
end
end
...
for i=1,#variableTable do
if variableTable[i][1] == name then
variableTable[i][2] = newValue
end
end
end

function queueSet(step,modName,target,plr)
local player = plr
if player == nil then
player = -1
end
addToList(queueSetTable,{step,modName,target,player})
end

function queueSetP(step,modName,target,plr)
local player = plr
if player == nil then
player = -1
end
addToList(queueSetPTable,{step,modName,target,player})
end

function queueEase(step,endStep,modName,percent,stl,plr,startVal)
local player = plr
if player == nil then
player = -1
end
...
addToList(queueEaseTable,{step,endStep,modName,percent,style,player,startVal})
end

function queueEaseP(step,endStep,modName,percent,stl,plr,startVal)
local player = plr
if player == nil then
player = -1
end
...
addToList(queueEasePTable,{step,endStep,modName,percent,style,player,startVal})
end
--[[
queueSet props
...
for i = 1, #queueSetTable do
if curStep == queueEaseTable[i][1] then
if queueEaseTable[i][1] == 1 then
end
end
end
end]]
